...
import { PrismaClient, ScrapeLog } from '@prisma/client';
import { PrismaBetterSqlite3 } from '@prisma/adapter-better-sqlite3';
import { registry } from '../scraper/registry.js';
import { upsertTournaments, ScrapedTournamentInput } from '../services/tournamentService.js';
  ...
  try {
    ...
    for (const source of sources) {
      try {
        ...
        const input: ScrapedTournamentInput[] = tournaments.map((t) => ({
          name: t.name,
...
export async function getLastScrapeStatus(): Promise<ScrapeLog | null> {
  return prisma.scrapeLog.findFirst({
    orderBy: { startTime: 'desc' },
  });
}

export async function getScrapeHistory(limit: number = 10): Promise<ScrapeLog[]> {
